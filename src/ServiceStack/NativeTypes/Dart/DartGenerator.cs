...
namespace ServiceStack.NativeTypes.Dart
{
    public class DartGenerator
    {
        ...
        public string GetCode(MetadataTypes metadata, IRequest request)
        {
            ...
            sb.AppendLine("*/");
            sb.AppendLine();
            sb.AppendLine("import 'dart:async';");
        ...
        private string AppendType(ref StringBuilderWrapper sb, MetadataType type, string lastNS, List<MetadataType> allTypes, CreateTypeOptions options)
        {
            ...
            {
                sb.AppendLine("class {0} {{".Fmt(Type(type.Name, type.GenericArgs)));
                sb = sb.Indent();
                sb.AppendLine("final _value;");
                sb.AppendLine("const {0}._internal(this._value);".Fmt(type.Name));
                sb.AppendLine("toString() => '{0}.$_value';".Fmt(type.Name));
                if (type.EnumNames != null)
                {
                    for (var i = 0; i < type.EnumNames.Count; i++)
                    {
                        ...
                        sb.AppendLine("static const {0} = const {1}._internal('{2}');".Fmt(name,type.Name,name));
                    }
                }
            ...
            else
            {
                ...
                sb.AppendLine("{0}class {1}".Fmt(partial, Type(type.Name, type.GenericArgs)));
                ...
                if (inheritsList.Count > 0)
                    sb.AppendLine(" implements {0} {{".Fmt(string.Join(", ", inheritsList.ToArray())));
                else
                    sb.Append("{");
                sb = sb.Indent();
        ...
        private void AddConstuctor(StringBuilderWrapper sb, MetadataType type, CreateTypeOptions options)
        {
            ...
            if (addVersionInfo)
            {
                ...
                sb.AppendLine("{0}int Version;".Fmt(@virtual));
                sb.AppendLine();
            }

            sb.AppendLine("{0}() {{".Fmt(NameOnly(type.Name)));
            sb = sb.Indent();
        ...
        public void AddProperties(StringBuilderWrapper sb, MetadataType type)
        {
            ...
            if (type.Properties != null)
            {
                foreach (var prop in type.Properties)
                {
                    ...
                    sb.AppendLine("{0}{1} {2};".Fmt(@virtual, propType, prop.Name.SafeToken()));
                }
            }
                    ...
                    || type.Properties.All(x => x.Name != "ResponseStatus")))
            {
                ...
                sb.AppendLine("{0}ResponseStatus ResponseStatus;".Fmt(@virtual));
            }
                    ...
                    || type.Properties.All(x => x.Name != "ExtensionData")))
            {
                ...
                sb.AppendLine("{0}ExtensionDataObject ExtensionData;".Fmt(@virtual));
            }
        }
        ...
        private string TypeAlias(string type, bool includeNested = false)
        {
            ...
            string typeAlias;
            Config.DartTypeAlias.TryGetValue(type, out typeAlias);
